...
export function DataTable({
    ...
    () => [
      {
        accessorKey: "address",
        header: ({ column }) =>
          renderSortableHeader(column, "Validator Address", sortableColumns),
        cell: ({ row }) => (
          <div className="flex flex-row gap-2.5 items-center">
            ...
            <HashLink
              hash={row.getValue("address") as string}
              type="address"
              sliceLength={4}
            />
          </div>
  ...
  const filterOptions = React.useMemo(() => {
    if (!filterColumn || !data?.validators) return [];
    const options = new Set(
      data.validators.map((item) =>
        String(item[filterColumn as keyof Validator])
      )
    );
